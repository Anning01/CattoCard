...
const providerTemplates: Record<string, {
  ...
  wechat: {
    name: '微信支付(Native)',
    description: '微信扫码支付，适用于 PC 端，用户扫码完成支付',
    fields: [
      {
        key: 'appid',
        label: 'AppID',
      ...
      },
      {
        ...
        type: 'password',
        required: true,
        tip: 'apiclient_key.pem 文件内容（商户证书私钥）',
      },
      {
        key: 'notify_url',
        label: '回调地址',
        type: 'text',
        required: true,
        placeholder: 'https://yourdomain.com/api/v1/payment/wechat/callback',
        tip: '微信支付异步通知地址，必须是 HTTPS',
      },
      {
        key: 'timeout',
        label: '请求超时(秒)',
